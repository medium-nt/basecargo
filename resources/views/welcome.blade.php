    ...
    <form action="{{ route('login') }}" method="GET" class="space-y-6">
        ...
        @if (Illuminate\Support\Facades\App::environment(['local']))
            <div class="mb-1">
                ...
                <a href="{{ route('users.autologin', ['email' => '2@2.ru']) }}" class="mr-3 bg-gray-100 text-gray-500">Агент</a>
                <a href="{{ route('users.autologin', ['email' => '3@3.ru']) }}" class="mr-3 bg-gray-100 text-gray-500">Клиент</a>
                <a href="{{ route('users.autologin', ['email' => '4@4.ru']) }}" class="mr-3 bg-gray-100 text-gray-500">Менеджер</a>
            </div>
        @endif
    </form>
</div>
